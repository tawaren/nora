...
@ReportPolymorphism
@NodeInfo(shortName = "|")
public abstract class OrNode extends NoraNode {
    ...
    public NoraNode specialise(SpecFrame frame) throws Exception {
        ...
        if (leftSpec instanceof ConstNode & rightSpec instanceof ConstNode) {
            ...
            return new CacheNode(OrNodeGen.create(cn1.liftCache(), cn2.liftCache()));
        }
        return OrNodeGen.create(leftSpec, rightSpec);
    }
    ...
    @Override
    public OrNode cloneUninitialized() {
        CompilerAsserts.neverPartOfCompilation();
        return OrNodeGen.create(getLeft().cloneUninitialized(),getRight().cloneUninitialized());
    }
