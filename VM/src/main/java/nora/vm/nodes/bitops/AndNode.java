...
@ReportPolymorphism
@NodeInfo(shortName = "&")
public abstract class AndNode extends NoraNode {
    ...
    public NoraNode specialise(SpecFrame frame) throws Exception {
        ...
        if(leftSpec instanceof ConstNode & rightSpec instanceof ConstNode){
            ...
            return new CacheNode(AndNodeGen.create(cn1.liftCache(), cn2.liftCache()));
        }
        return AndNodeGen.create(leftSpec, rightSpec);
    }
    ...
    @Override
    public AndNode cloneUninitialized() {
        CompilerAsserts.neverPartOfCompilation();
        return AndNodeGen.create(getLeft().cloneUninitialized(),getRight().cloneUninitialized());
    }
