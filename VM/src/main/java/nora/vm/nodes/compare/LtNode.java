...
@ReportPolymorphism
@NodeInfo(shortName = "<")
public abstract class LtNode extends NoraNode {
    ...
    public NoraNode specialise(SpecFrame frame) throws Exception {
        ...
        if(leftRes instanceof ConstNode && rightRes instanceof ConstNode){
            ...
            return new CacheNode(LtNodeGen.create(cn1.liftCache(), cn2.liftCache()));
        } else {
            ...
            return LtNodeGen.create(leftRes, rightRes);
        }
    }
    ...
    @Override
    public LtNode cloneUninitialized() {
        CompilerAsserts.neverPartOfCompilation();
        return LtNodeGen.create(getLeft().cloneUninitialized(),getRight().cloneUninitialized());
    }
