...
@ReportPolymorphism
@NodeInfo(shortName = "<=")
public abstract class LteNode extends NoraNode {
    ...
    public NoraNode specialise(SpecFrame frame) throws Exception {
        ...
        if(leftRes instanceof ConstNode && rightRes instanceof ConstNode){
            ...
            return new CacheNode(LteNodeGen.create(cn1.liftCache(), cn2.liftCache()));
        } else {
            ...
            return LteNodeGen.create(leftRes, rightRes);
        }
    }

    @Override
    public String toString() {
        return "Lte(" +getLeft()+"," + getRight() +")";
    }

    @Override
    public LteNode cloneUninitialized() {
        CompilerAsserts.neverPartOfCompilation();
        return LteNodeGen.create(getLeft().cloneUninitialized(),getRight().cloneUninitialized());
    }
