...
@ReportPolymorphism
@NodeInfo(shortName = ">")
public abstract class GtNode extends NoraNode {
    ...
    public NoraNode specialise(SpecFrame frame) throws Exception {
        ...
        if(leftRes instanceof ConstNode && rightRes instanceof ConstNode){
            ...
            return new CacheNode(GtNodeGen.create(cn1.liftCache(), cn2.liftCache()));
        } else {
            ...
            return GtNodeGen.create(leftRes, rightRes);
        }
    }
    ...
    @Override
    public GtNode cloneUninitialized() {
        CompilerAsserts.neverPartOfCompilation();
        return GtNodeGen.create(getLeft().cloneUninitialized(),getRight().cloneUninitialized());
    }
