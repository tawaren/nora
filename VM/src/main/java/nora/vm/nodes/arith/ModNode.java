...
@ReportPolymorphism
@NodeInfo(shortName = "%")
public abstract class ModNode extends NoraNode {
    ...
    public NoraNode specialise(SpecFrame frame) throws Exception {
        ...
        if(leftRes instanceof ConstNode && rightRes instanceof ConstNode){
            ...
            return new CacheNode(ModNodeGen.create(cn1.liftCache(), cn2.liftCache()));
        } else {
            return ModNodeGen.create(leftRes, rightRes);
        }
    }

    @Override
    public String toString() {
        return "Mod(" +getLeft()+"," + getRight() +")";
    }

    @Override
    public ModNode cloneUninitialized() {
        CompilerAsserts.neverPartOfCompilation();
        return ModNodeGen.create(getLeft().cloneUninitialized(),getRight().cloneUninitialized());
    }
