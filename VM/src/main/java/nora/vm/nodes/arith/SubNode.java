...
@ReportPolymorphism
@NodeInfo(shortName = "-")
public abstract class SubNode extends NoraNode {
    ...
    public NoraNode specialise(SpecFrame frame) throws Exception {
        ...
        if(leftRes instanceof ConstNode && rightRes instanceof ConstNode){
            ...
            return new CacheNode(SubNodeGen.create(cn1.liftCache(), cn2.liftCache()));
        } else {
            return SubNodeGen.create(leftRes, rightRes);
        }
    }

    @Override
    public String toString() {
        return "Sub(" +getLeft()+"," + getRight() +")";
    }

    @Override
    public SubNode cloneUninitialized() {
        CompilerAsserts.neverPartOfCompilation();
        return SubNodeGen.create(getLeft().cloneUninitialized(),getRight().cloneUninitialized());
    }
